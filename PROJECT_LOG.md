...
## 2026-09-08 — Трек-подход узаконен в каноне; хоткей-тумблер
- Владелец подтвердил: `app/dictate.py` работает вживую на ПК1.
- Канон обновлён: CONTEXT.md («Треки» — A1 свой скрипт ОСНОВНОЙ, A2 GUI резерв),
  TASK.md переписан на трек-подход как официальный (спека + статус ПК1 done, паритет ПК2).
- UX по просьбе владельца: убрана отдельная клавиша F8 — `ctrl+space` стал тумблером
  (нажать — старт, нажать ещё — стоп+вставка). Двухклавишный режим доступен через `--stop-key`.
  Проверено запуском: toggle стартует/стопит мик и ставит клип в очередь.

## 2026-09-08 — Живой тест на ПК1: hold-to-record, диктовка работает
- Первый живой прогон вскрыл граблю: тумблер на зажатой `ctrl+space` рвал запись —
  автоповтор Windows re-fire'ил хоткей каждые ~0.3-0.5 c (старт/стоп), клипы <0.3 c
  резались порогом, ловилась тишина -> галлюцинации Whisper. Микрофон был ни при чём
  (mic_check.py: peak 0.175 при речи; добавлен как отдельный диагностический инструмент).
- Исправление: дефолт -> **hold-to-record** (зажать `ctrl+space` — запись, отпустить —
  транскрайб+вставка; start идемпотентен, стоп по отпусканию через watcher). Иммунно к
  автоповтору, совпадает с UX ПК2. `--toggle` и `--stop-key` оставлены опциями.
- Добавлена диагностика: лог устройства и уровня `peak=`, `--list-devices`/`--device-index`.
- ПОДТВЕРЖДЕНО ВЖИВУЮ: зажать Ctrl+Space, ~10 c речи -> 100 символов в Блокнот,
  transcribe 1.66 c, peak 0.18. Трек A1 (свой скрипт) на ПК1 ЗАКРЫТ.
- Канон синхронизирован: CONTEXT.md, TASK.md, REPORT.md — hold-to-record как дефолт.

## 2026-09-09 — ПК1: диктовка через свой скрипт + разнос знаний по DEV-NOTES
- GUI whisper-local на ПК1 виснет на transcribing (проглоченное «cublas64_12.dll not found», как на ПК2). Решение — вариант A: своя диктовка на faster-whisper в обход обёртки.
- Уроки разнесены: универсальные — в общий DEV-NOTES.md §18–22; проектные (whisper/CT2, cuBLAS, микрофон, хоткеи) — в DEV-NOTES-STT-WHISPER-LOCAL.md (канон в этом репо).
- Схема: канон один в git, в G:\AI\DEV-NOTES\ — редирект с типом проектов. Машино-факты помечены [ПК1]/[ПК2].
