...
и виснет на «transcribing»). Статус: **ГОТОВО, подтверждено живой диктовкой
(2026-09-08): зажать Ctrl+Space, ~10 c речи -> 100 символов вставлено в Блокнот,
transcribe 1.66 c, peak 0.18.**

**Хоткей — hold-to-record `ctrl+space`** (зажать — запись, отпустить -> транскрайб ->
вставка). Разбор граблей на живом тесте:
- Тумблер на одной клавише `ctrl+space` НЕ годится: автоповтор Windows у зажатого
  модификатора re-fire'ит хоткей каждые ~0.3-0.5 c -> старт/стоп чередуются, запись
  рвётся на куски <0.3 c (все режутся порогом), в итоге ловится момент тишины ->
  галлюцинация Whisper («Продолжение следует…», «Субтитры сделал…»).
- Микрофон был НИ ПРИ ЧЁМ: `app/mic_check.py` показал живой сигнал (peak 0.175 при речи).
- Решение: hold-to-record (start идемпотентен, стоп — по отпусканию клавиши через
  watcher-поток) — иммунно к автоповтору. Это же UX работает на ПК2.
- Диагностика в скрипте: лог устройства и `peak=` уровня; `--list-devices`/`--device-index`;
  отдельный `app/mic_check.py`. Альт-режимы: `--toggle` (тап/тап, лучше f9), `--stop-key f8`.
...
**Живой тест пройден** (владелец, 2026-09-08): Блокнот -> зажать Ctrl+Space ->
русская фраза -> отпустить -> текст вставлен. Осталось: паритет на ПК2.
