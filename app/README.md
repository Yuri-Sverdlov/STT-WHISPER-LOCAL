...
## Хоткеи
- **Ctrl+Space (зажать)** — hold-to-record: держишь — идёт запись, отпустил →
  транскрайб → вставка под курсор (буфер + эмуляция Ctrl+V). Проверенная схема ПК2,
  иммунна к автоповтору клавиши.
...
```powershell
# тумблер (тап/тап) -- лучше на одиночной клавише без модификатора
pwsh -File .\app\run-dictate.ps1 --toggle --start-key f9
# двухклавишный: старт/стоп разными клавишами
...
## Диагностика
- **`no audio captured` / галлюцинации («Субтитры сделал…», «Продолжение следует…»)**
  — микрофон отдаёт тишину, потому что его держит другое приложение. Самое частое —
  **Wispr Flow в трее** (его и заменяем): полностью закрыть (Quit), не сворачивать.
- Лог показывает `peak=` — уровень сигнала. `peak<0.01` = тишина (мьют/занят/не тот вход).
- Выбор микрофона вручную:
  ```powershell
  pwsh -File .\app\run-dictate.ps1 --list-devices           # список устройств
  pwsh -File .\app\run-dictate.ps1 --device-index 1          # явный вход (напр. LifeCam)
  ```
- Хоткей не срабатывает / срабатывает чужая программа — проверь, не занят ли `ctrl+space`
  другим приложением; можно сменить: `--start-key f9` и т.п.
